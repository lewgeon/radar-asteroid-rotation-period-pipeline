...
| `visibility.sample_step_s` | 本地几何可见性采样步长（不是 Horizons 查询步长）。仅当地测站使用；两端都是自定义直角坐标时该段应省略 |
| `adc_window_duration_s` | `observation_info.npz` 输出：Run 级连续 ADC 窗时长。不是调度时长 `schedule.run_duration_s` |
| `receive_centroid_span_s` | `observation_info.npz` 输出：质心回波接收时刻跨度 |
| `radar_system.switch_time_s` | 单站收发切换所需时间 |
| `radar_system.safety_margin_s` | 切换后再留的安全死区，避免最早回波撞上发射占用 |
| `target.extent_path_m` | 采集路径窗：相对质心参考回波，两侧各允许的最大额外双程路径。未知尺度、星历误差和余量都写在这一个数里，时延为该长度除以光速。Chirp 用它决定脉冲保存行宽度；连续波不使用。GUI 标签是「采集路径窗」，画在「接收采样」卡片，可选 m 或 km |
...
| `transmit.prf_hz` | Chirp 脉冲重复频率。不是接收 ADC 采样率 |
| `transmit.pulse_width_s` | Chirp 单个发射脉冲的持续时间 |
| `echo.waveform.type` | 射频波形类型：`continuous_wave` 或 `chirp_pulse_train`。由 GUI 顶部「发射波形」写入；须与观测事件源一致 |

## 明确删除或不再使用的字段

- `receiver_sampling.pre_guard_s` / `post_guard_s`（载入时并入 `target.extent_path_m`：取原路径长度与 $c$ 乘保护时间中的较大值，然后删除这两个键）
- `observation.waveform`（PRF/脉宽改写 `observation.transmit`，类型改写 `echo.waveform.type`）
