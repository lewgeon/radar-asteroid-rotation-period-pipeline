...
最大化窗口后模块仍保持双栏，不再切换成另一套卡片排列。结果预览位于窗口右侧的
可关闭侧栏：完成任一阶段后自动打开，也可用左侧“显示/隐藏结果侧栏”按钮控制。
关闭侧栏后参数与日志会使用释放出来的宽度。下方只保留执行日志；可以拖动参数与
日志之间的分隔条。极窄参数区才会回退单列，窗口缩放不会提交尚未输入完的数值。

卡片标题使用浅蓝标题带与深蓝加粗字；“形状模型、散射指数、信噪比”等同级字段
使用一致的字体。自转轴、初始坐标、速度、散射指数和斑块方向统一使用纵向关系线
表示子模块：模块标题位于左侧，浅色竖线将标题与右侧下属条目分开，条目名称位于
输入框上方并纵向排列。
卡片内顶层字段保持一致的竖向间距。数值与单位共用等高边框，可选单位以右侧箭头
区分，固定单位沿用相同配色。单位选择仍表示当前输入数值的单位，保存时换算为标准单位。

滚动条采用深色滑块与浅色轨道。所有布尔参数统一使用勾选框：选中时为蓝底白勾，
未选中时为白底深边框，不再使用“是/否”下拉框。
下拉列表按控件位置展开，不再以当前选项作为弹出位置锚点；只有屏幕底部空间不足时
才向上展开。Windows 下优先使用原生样式及其展开动画，系统不支持该效果时直接显示列表。

回波页左栏依次为“计算设置、雷达参数”，右栏依次为“目标参数、散射特性”。
“雷达参数”包含载频、波形、信噪比和噪声种子；“目标参数”包含形状模型；
“散射特性”包含散射指数和可选的散射热点。反演页分为“时频分析”和“周期搜索”。
...
旧 JSON 无需迁移。这里随时间变化的是测站在地心惯性系中的位置，而不是经纬度和高度本身。
...
观测解算会自动把接收时间轴、发射/散射时刻、收发视线和距离保存到
`runs/<实验名>/observation_info.npz`。勾选“复用已有视线向量”并选择该文件后，
点击执行会校验必需数组及长度，跳过观测解算并切换到回波仿真。随后生成的回波配置
会直接引用所选文件，适合保持观测配置不变、只更换波形或反演算法的实验。

“另存当前向量”在观测解算完成后可用，用于把当前 NPZ 复制到长期保存位置。
复用前仍应确认文件确实来自相同的目标、测站、时间轴和求解器配置；GUI 的结构校验
不能替代科学配置核对。复用路径只保存在 GUI 状态中，不写入顶层业务配置。
...
| 波形类型 | `type` | 发射波形 | 连续波或 Chirp 脉冲序列；选择 Chirp 后展开相关字段。 |
| 幅度 | `amplitude` | CW 波形幅度 | 连续波模式下为常数幅度。 |
| 脉冲宽度 | `pulse_width_s` | 线性调频脉冲宽度 | Chirp 模式使用。 |
| 带宽 | `bandwidth_hz` | 线性调频带宽 | Chirp 模式使用。 |
| 脉冲重复间隔 | `pri_s` | PRI | Chirp 模式使用。 |
| 首脉冲起点 | `first_pulse_start_s` | 第一个脉冲起始时刻 | Chirp 模式使用。 |
| 信噪比 | `snr_db` | 复高斯噪声 SNR，单位 dB | `null` 表示不加噪声。 |
| 噪声种子 | `seed` | 噪声随机数种子 | 仅在添加复高斯噪声时使用；固定后噪声可重复。 |
