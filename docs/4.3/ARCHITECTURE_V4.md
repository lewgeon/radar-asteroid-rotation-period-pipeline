...
- Chirp：`schedule` + `transmit` + `radar_system` + `receiver_sampling`
- CW：`receive`（连续采样）
- 二者不能同时出现
- 射频类型只写 `echo.waveform.type`，与事件源对齐；PRF/脉宽写 `observation.transmit`

## 示例

开发验证见 `configs/chirp_point_target_test.json`。正式示例配置待重新设计。
